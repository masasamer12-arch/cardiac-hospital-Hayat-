(function () {
  ...
  function renderForm() {
    return [
      ...
      "</div>",
      '<form id="vitalForm" class="grid gap-4 md:grid-cols-5" novalidate>',
  ...
  function validateBloodPressure(value) {
    ...
    if (!bpRegex.test(trimmed)) {
      return { valid: false, message: "صيغة ضغط الدم غير صحيحة. يرجى إدخال الصيغة التالية: الانقباضي/الانبساطي (مثال: 120/80)" };
    }
    var parts = bpRegex.exec(trimmed);
    var systolic = parseInt(parts[1], 10);
    var diastolic = parseInt(parts[2], 10);
    if (systolic < 50 || systolic > 300) {
      return { valid: false, message: "قيمة الضغط الانقباضي غير منطقية. يجب أن تكون بين 50 و300." };
    }
    if (diastolic < 30 || diastolic > 200) {
      return { valid: false, message: "قيمة الضغط الانبساطي غير منطقية. يجب أن تكون بين 30 و200." };
    }
    if (diastolic >= systolic) {
      return { valid: false, message: "الضغط الانبساطي يجب أن يكون أقل من الضغط الانقباضي." };
    }
  ...
  function bindEvents() {
    document.getElementById("vitalForm").addEventListener("submit", function (event) {
      ...
      var bpInput = document.getElementById("bloodPressure");
      var bpValidation = validateBloodPressure(bpInput.value);
      if (!bpValidation.valid) {
        alert(bpValidation.message);
        bpInput.focus();
        bpInput.classList.add("border-rose-500");
        return;
      }
      bpInput.classList.remove("border-rose-500");

      const newRecord = {
        id: "local-vital-" + Date.now(),
        patientId: document.getElementById("patientId").value,
        heartRate: Number(document.getElementById("heartRate").value),
        bloodPressure: bpValidation.value,

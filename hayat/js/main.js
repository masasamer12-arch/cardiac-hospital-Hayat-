(function () {
  const routeAccess = {
    ...
    "patient-details": ["doctor", "nurse", "receptionist"],
    vitals: ["doctor", "nurse"],
  ...
  const pageTitles = {
    ...
    "nurse-dashboard": "لوحة تحكم الممرضة",
  ...
  async function getPatients() {
    const basePatients = await fetchJson("./data/patients.json", "فشل تحميل بيانات المرضى");
    const localPatients = getLocalPatients();
    return basePatients.concat(localPatients);
  }

  function getLocalPatients() {
    return readArray(localKeys.patients);
  }

  function saveLocalPatients(patients) {
    writeArray(localKeys.patients, patients);
  }

  function getLocalPatientById(id) {
    const localPatients = getLocalPatients();
    return localPatients.find(function (p) {
      return p.id === id;
    });
  }

  function updateLocalPatient(id, updates) {
    const localPatients = getLocalPatients();
    const index = localPatients.findIndex(function (p) {
      return p.id === id;
    });
    if (index !== -1) {
      localPatients[index] = Object.assign({}, localPatients[index], updates);
  ...
  function addLocalPatient(patientData) {
    const localPatients = getLocalPatients();
    const maxId = localPatients.reduce(function (max, p) {
  ...
  async function getVitalRecords() {
    const patients = await getPatients();
    const baseVitals = patients
      .filter(function (patient) {
        return patient.heartRate != null && patient.bloodPressure && patient.oxygenLevel != null;
      })
  ...
  function pendingBadge() {
    return badge("بانتظار إكمال البيانات", "bg-orange-50 text-orange-700 ring-orange-200");
  }

  function isPendingPatient(patient) {
    return patient.status === "pending" || !patient.bloodType || !patient.condition || !patient.diagnosis;
  }
  ...
  async function initProtectedPage() {
    ...
    if (!allowedRoles.includes(role)) {
      const defaultPage = CardiacAuth.roleDashboards[role] || "dashboard.html";
